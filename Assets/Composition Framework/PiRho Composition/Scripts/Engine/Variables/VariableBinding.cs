...
namespace PiRhoSoft.Composition
{
	...
	public abstract class VariableBinding : MonoBehaviour
	{
		...
		public bool ResolveAny<T>(IVariableCollection variables, VariableReference reference, out T result)
		{
		...
		public bool Resolve(IVariableCollection variables, VariableReference reference, out ObjectVariableCollection result)
		{
			var value = reference.GetValue(variables);

			if (value.TryGetCollection(out result))
				return true;
		...
		public bool Resolve(IVariableCollection variables, VariableReference reference, out object result)
		{
			var value = reference.GetValue(variables);

			if (value.TryGetObject(out result))
				return true;
