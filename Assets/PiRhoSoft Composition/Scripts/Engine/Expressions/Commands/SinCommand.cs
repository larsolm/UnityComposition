...
{
	public class SinCommand : Command
	{
		public override VariableValue Evaluate(IVariableStore variables, string name, List<Operation> parameters)
		{
			if (parameters.Count == 1)
			{
				...
				switch (result.Type)
				{
					case VariableType.Integer: return VariableValue.Create(Mathf.Sin(result.Integer));
					case VariableType.Number: return VariableValue.Create(Mathf.Sin(result.Number));
