		...
		public VariableValue Evaluate(IVariableStore variables, string name, List<Operation> parameters)
		{
			if (parameters.Count == 3)
			{
				...
				if (a.Type != VariableType.Integer && a.Type != VariableType.Number)
					throw new CommandEvaluationException(name, Command.WrongParameterType1Exception, a.Type, 0, VariableType.Number);

				if (b.Type != VariableType.Integer && b.Type != VariableType.Number)
					throw new CommandEvaluationException(name, Command.WrongParameterType1Exception, b.Type, 1, VariableType.Number);

				if (t.Type != VariableType.Integer && t.Type != VariableType.Number)
					throw new CommandEvaluationException(name, Command.WrongParameterType1Exception, t.Type, 2, VariableType.Number);
