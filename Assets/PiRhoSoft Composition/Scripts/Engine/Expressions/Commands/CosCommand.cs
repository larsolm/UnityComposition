...
{
	public class CosCommand : ICommand
	{
		public VariableValue Evaluate(IVariableStore variables, string name, List<Operation> parameters)
		{
			if (parameters.Count == 1)
			{
				...
				switch (result.Type)
				{
					case VariableType.Integer: return VariableValue.Create(Mathf.Cos(result.Integer));
					case VariableType.Number: return VariableValue.Create(Mathf.Cos(result.Number));
