		...
		private void BuildMethodList()
		{
			if (_node.TargetType != null)
			{
				_methods = _node.TargetType.GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly)
					...
					.Where(method => method.ReturnType == typeof(void) || VariableValue.GetType(method.ReturnType) != VariableType.Other)
					.Where(method => !method.GetParameters().Any(parameter => VariableValue.GetType(parameter.ParameterType) == VariableType.Other))
